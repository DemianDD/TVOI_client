...
const Tips = () => {
    ...
    return (
        <div className='bg-white w-full pb-4 flex justify-center relative'>
            <div>
                <div className='pt-3'>
                    <LazyLoadImage src={randomTip.image} className='2xl:w-[250px] w-[200px]' effect="blur" PlaceholderSrc="defaults/logoclear.png"/>
                </div>
                <div
                    key={randomTip.id}
                    className="absolute bottom-5 left-0 w-full text-center bg-white text-medium lg:text-base text-sm gilroy flex items-center p-2 px-4 rounded-3xl text-[#919191] shadow-[#ccc] shadow-md"
                >
                    {randomTip.title}
                </div>
            </div>
